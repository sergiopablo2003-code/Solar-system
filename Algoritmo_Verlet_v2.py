...
b_registrado=np.zeros(len(x),dtype=bool)#vector de valores booleanos, indica si ya se ha registrado b
...
while t_c<tf:
      ...
      for k in range(0,len(x)):
         ...
         # Si la coordenada y se vuelve negativa y la anterior era positiva se añade una vuelta más.
         if y[k] < 0 and P_y[len(P_y[:,k])-1,k]>0:
             contadores[k]=contadores[k]+1
             tiempos[k]=t_c

         if abs(x[k])< 4e-3 and not b_registrado[k]:
           b[k]=abs(y[k])
           b_registrado[k]=True
...
for k in range (1,len(x)):
    if b[k]>a[k]:
        e=np.sqrt(1-(a[k]**2/b[k]**2))
    
    if a[k]>b[k]:
        e=np.sqrt(1-(b[k]**2/a[k]**2))
    print(f"excentricidad de {labels[k]}:", e)
...
#graficamos las orbitas de los planetas
